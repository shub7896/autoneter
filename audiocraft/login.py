...
def signup_page():
    ...
    with st.expander("Why login signup need ?"):
        st.write("Apps often require a login for a variety of reasons, including: User authentication: A login system allows the app to verify the user's identity and ensure that only authorized users can access the app's features and data.")
    st.title('Sign Up')
...
def update_profile(new_username, new_password):
    try:
        connection = connect_to_database()
        cursor = connection.cursor()
        ...
        if new_username:
            ...
            data = (new_username, st.session_state.user_id)
            cursor.execute(query, data)
        if new_password:
            query = "UPDATE users SET password = %s WHERE id = %s"
            data = (hashed_password, st.session_state.user_id)
            cursor.execute(query, data)
